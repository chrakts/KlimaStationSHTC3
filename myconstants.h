/*
 * constants.h
 *
 * Created: 24.05.2018 19:24:50
 *  Author: chrak_2
 */


#ifndef MYCONSTANTS_H_
#define MYCONSTANTS_H_

#define BROADCAST "BR"

#define SENSOR_READY 0

enum{NOTHING_CLIMA_TODO=SENSOR_READY,WAKEUP,WAIT_WAKEUP,
	START_CONVERSION,WAIT_CONVERSION,READ_CONVERSION,WAIT_READ,
	CALC_CONVERSION0,CALC_CONVERSION1,CALC_CONVERSION2};

enum{NOTHING_LAST_TODO=SENSOR_READY,WAIT_LAST,READY_LAST};

enum{KLIMASENSOR=0,LASTSENSOR};

enum{FAN_STATUS_OFF,FAN_STATUS_1,FAN_STATUS_2,FAN_STATUS_AUTO,FAN_STATUS_UNVALID = 255};

#define REPORT_BETWEEN_SENSORS 50
#define REPORT_BETWEEN_BLOCKS  800
#define WAIT_AFTER_LAST_SENSOR  300

#ifdef FENSTER
  #ifdef DIMMERA
    enum{TEMPREPORT=0,HUMIREPORT,ABSHUMIREPORT,DEWPOINTREPORT,FENSTERREPORT0,FENSTERREPORT1,DIMMERREPORTA,LASTREPORT};
  #else
    enum{TEMPREPORT=0,HUMIREPORT,ABSHUMIREPORT,DEWPOINTREPORT,FENSTERREPORT0,FENSTERREPORT1,LASTREPORT};
  #endif // DIMMERA
#else
enum{TEMPREPORT=0,HUMIREPORT,ABSHUMIREPORT,DEWPOINTREPORT,LASTREPORT};
#endif // FENSTER


#define NUMBER_OF_TEMPSENSORS 2
#define NUMBER_OF_SENSORS_NAME_LENGTH 12
#define NUMBER_STORED_SENSORS NUMBER_OF_TEMPSENSORS

#endif /* MYCONSTANTS_H_ */
